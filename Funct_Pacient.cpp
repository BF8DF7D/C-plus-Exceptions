...
Pacient::Pacient() {
	...
	do {
		...
		std::cin >> Medical_Card;
		False_Input_Value = (Medical_Card < Minimum_number || Medical_Card > Maximum_number)
	...
	enum Limit_Value_for_diagnosis {
...
void Pacient::PrintInfo() {
	std::string name = GetFIO().GetFullName();
	std::cout << " <Персональные данные>" << std::endl;
	std::cout << " ФИО пациента            : " << name << std::endl;
